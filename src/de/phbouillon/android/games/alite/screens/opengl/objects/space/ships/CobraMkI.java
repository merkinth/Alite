...
public class CobraMkI extends SpaceObject {
	...
	private static final Vector3f HUD_COLOR_AS_TRADER = new Vector3f(0.94f, 0.94f, 0.0f);
	private static final Vector3f HUD_COLOR_AS_ENEMY = new Vector3f(0.55f, 0.67f, 0.94f);
	...
	public CobraMkI(Alite alite) {
		this(alite, ObjectType.Trader);
	}

	private CobraMkI(Alite alite, ObjectType type) {
		super(alite, "Cobra Mk I", type);
		setName(R.string.so_cobra_mk_i);
		...
		maxRollSpeed      = 2.000f;
		hullStrength      = type == ObjectType.EnemyShip ? 140f : 72.0f;
		hasEcm            = true;
		cargoType         = 0;
		aggressionLevel   = 5;
		escapeCapsuleCaps = 3;
		bounty            = type == ObjectType.EnemyShip ? 40 : 0;
		score             = 70;
		legalityType      = 0;
		maxCargoCanisters = 0;
	...
	public static SpaceObject createCobraMkIAsEnemy(Alite alite) {
		return new CobraMkI(alite, ObjectType.EnemyShip);
	}
	...
	protected void init() {
		...
		if (Settings.engineExhaust) {
			addExhaust(new EngineExhaust(this, 18, 13, 240, -30, 0, 0));
			addExhaust(new EngineExhaust(this, 18, 13, 240,  30, 0, 0));
		}
		initTargetBox();
	}
	...
	public Vector3f getHudColor() {
		return getType() == ObjectType.Trader ? HUD_COLOR_AS_TRADER : HUD_COLOR_AS_ENEMY;
	}
